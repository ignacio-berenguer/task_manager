...
Excel columns are normalized to lowercase and mapped to database columns:

| Excel Column | DB Column | Normalization |
|-------------|-----------|---------------|
| Tarea | `tarea` | Multiline text normalization |
| Responsable | `responsable` | — |
| Descripcion | `descripcion` | Multiline text normalization |
| Fecha_NBA | `fecha_siguiente_accion` | Date normalization (Excel serial, text, datetime) |
| Tema | `tema` | Multiline text normalization (empty → NULL) |
| Estado | `estado` | — |
| Notas | `notas_anteriores` | Multiline text normalization |

Missing columns are logged as warnings and default to NULL. The `tarea_id` primary key is auto-generated by PostgreSQL (SERIAL).
...
| `/search` | Private | Task search with labeled filters, lateral sidebar (xl+), column filter popovers (funnel icon), active filter tags (removable badges), quick filter "Proxima semana" (toggles fecha_siguiente_accion date range: today + 6 days), sortable/reorderable columns, colored estado tags, sticky title bar, sticky results bar and table header (XL+, dynamic height via ResizeObserver), inline detail accordion, side drawer quick view, quick action buttons (add accion, change fecha), keyboard shortcuts, new tarea dialog, persistent state across navigation (filters, results, pagination, sort, scroll position). Mobile-optimized: icon-only buttons, responsive drawer grid, touch-friendly tap targets |
| `/detail/:tarea_id` | Private | Task header with tarea name prominent + tarea_id muted, estado + responsable + fecha_siguiente_accion badges + cambiar fecha button, compact acciones CRUD table (sorted desc, sticky headers), add accion dialog, notas accordion (collapsed), datos accordion (collapsed), Ctrl+Shift+F shortcut, back navigation with state preservation. Mobile-optimized: card layout for acciones, responsive accordion padding, icon-only header buttons |
