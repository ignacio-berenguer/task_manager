...
class PortfolioAPIClient:
    """Synchronous HTTP client for the Portfolio Digital FastAPI backend."""
    ...
    def search(
        ...
        response = self.client.post(f"/{prefix}/search", json=body)
        return self._handle_response(response)

    def list_records(self, table: str, limit: int = 50, offset: int = 0) -> dict:
    ...
    def get_portfolio_records(self, table: str, portfolio_id: str) -> list:
        ...
        response = self.client.get(f"/{prefix}/portfolio/{portfolio_id}")
        return self._handle_response(response)

    def execute_sql(self, sql: str, max_rows: int = 500) -> dict:
        """POST /sql/execute — execute a read-only SQL query."""
        logger.debug("POST /sql/execute sql=%s", sql[:100])
        response = self.client.post("/sql/execute", json={"sql": sql, "max_rows": max_rows})
        return self._handle_response(response)
