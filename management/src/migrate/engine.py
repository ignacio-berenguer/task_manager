...
class TareasMigrationEngine:
    ...
    def migrate_tareas(self):
        ...
        # Insert in batches (tarea_id is auto-generated by DB)
        ...
        for i in range(0, len(mapped_df), batch_size):
            ...
            for _, row in batch.iterrows():
                try:
                    ...
                    col_names = ", ".join(cols)
                    cursor.execute(
                        f"INSERT INTO tareas ({col_names}) VALUES ({placeholders})",
                        vals,
                    )
                    inserted += 1
                except Exception as e:
                    errors += 1
                    LOG.warning(f"Error inserting tarea '{row.get('tarea', '?')}': {e}")
                    self.conn.rollback()
            self.conn.commit()
    ...
    def migrate_acciones_from_notas(self):
        ...
        for tarea_id, notas_text in tareas:
            ...
            for accion in acciones:
                try:
                    cursor.execute(
                        "INSERT INTO acciones_realizadas (tarea_id, accion, fecha_accion, estado) VALUES (%s, %s, %s, %s)",
