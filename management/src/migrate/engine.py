...
# Note: tarea_id is auto-generated (SERIAL PK) and not mapped from Excel
TAREAS_COLUMN_MAP = {
    "tarea": "tarea",
...
class TareasMigrationEngine:
    """Import tareas and acciones from Excel into PostgreSQL."""
    ...
    def migrate_tareas(self):
        ...
        # Map columns (tarea_id is auto-generated by DB, not from Excel)
        mapped_cols = {}
        ...
        if "tarea" not in mapped_cols:
            raise ValueError("Required column 'tarea' not found in Excel")
        ...
        # Fixed column set for bulk insert (all mapped DB columns)
        columns = list(TAREAS_COLUMN_MAP.values())
        col_names = ", ".join(columns)
        insert_sql = f"INSERT INTO tareas ({col_names}) VALUES %s"

        # Insert in batches using bulk execute_values (tarea_id is auto-generated by DB)
        ...
        for i in range(0, len(mapped_df), batch_size):
            ...
            try:
                execute_values(cursor, insert_sql, values_list, page_size=batch_size)
                self.conn.commit()
                batch_inserted = len(values_list)
                inserted += batch_inserted
                bulk_count += batch_inserted
                LOG.debug(f"Batch {batch_num}: inserted {batch_inserted} tareas (bulk)")
            except Exception as e:
                self.conn.rollback()
                LOG.warning(f"Bulk insert failed for batch {batch_num}, falling back to row-by-row: {e}")
                # Fallback: insert row by row to isolate bad rows
                for values in values_list:
                    try:
                        cursor.execute(
                            f"INSERT INTO tareas ({col_names}) VALUES ({', '.join(['%s'] * len(columns))})",
                            values,
                        )
                        self.conn.commit()
                        inserted += 1
                        ...
                        errors += 1
                        LOG.warning(f"Error inserting tarea (fallback): {row_err}")
                        self.conn.rollback()

        duration = time.time() - start
        rate = inserted / duration if duration > 0 else 0
        LOG.info(
            f"Tareas migration: {inserted} inserted, {errors} errors in {duration:.1f}s "
            f"({rate:.0f} rows/s, {batch_num} batches, {bulk_count} bulk + {fallback_count} fallback)"
        )
    ...
    def migrate_acciones_from_notas(self):
        ...
        # Collect all acciones into a flat list of tuples
        all_acciones = []
        completado_count = 0
        pendiente_count = 0
        ...
        LOG.info(f"Parsed {len(all_acciones)} acciones from {len(tareas)} tareas with notas")

        # Bulk insert in batches
        acciones_columns = "tarea_id, accion, fecha_accion, estado"
        insert_sql = f"INSERT INTO acciones_realizadas ({acciones_columns}) VALUES %s"
        batch_size = settings.BATCH_COMMIT_SIZE
        inserted = 0
        errors = 0
        bulk_count = 0
        ...
        for i in range(0, len(all_acciones), batch_size):
            batch = all_acciones[i:i + batch_size]
            batch_num += 1
            ...
            try:
                ...
                self.conn.commit()
                inserted += len(batch)
                bulk_count += len(batch)
                LOG.debug(f"Batch {batch_num}: inserted {len(batch)} acciones (bulk)")
            except Exception as e:
                self.conn.rollback()
                LOG.warning(f"Bulk insert failed for acciones batch {batch_num}, falling back to row-by-row: {e}")
                # Fallback: insert row by row to isolate bad rows
                for values in batch:
                    try:
                        cursor.execute(
                            f"INSERT INTO acciones_realizadas ({acciones_columns}) VALUES (%s, %s, %s, %s)",
                            values,
                        )
                        self.conn.commit()
                        inserted += 1
                        ...
                        errors += 1
                        LOG.warning(f"Error inserting accion for tarea '{values[0]}': {row_err}")
                        self.conn.rollback()
    ...
    def migrate_responsables(self):
        ...
        for i, valor in enumerate(responsables):
            try:
                cursor.execute(
                    "INSERT INTO responsables (valor, orden) VALUES (%s, %s) ON CONFLICT DO NOTHING",
                    (valor, i + 1),
                )
